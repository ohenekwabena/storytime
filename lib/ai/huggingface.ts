...
Include ${length.scenes} scenes with characters, settings, and dialogue. Make it simple and engaging.`;
  ...
  try {
    ...
    const structuredStory = convertToStructuredStory(
 ...
 */
function convertToStructuredStory(
  text: string,
  prompt: string,
  sceneCount: number,
  style: string
...
export async function generateCharacterImage(params: CharacterGenerationParams): Promise<Blob> {
  ...
  const prompt = `${stylePrompts[style]}, full body character design, ${age} ${gender}, ${name}, ${description}, ${traits.join(', ')}, white background, character sheet, front view, high quality, professional`;
...
export async function generateSceneBackground(params: SceneGenerationParams): Promise<Blob> {
  ...
  const { description, style, timeOfDay = 'afternoon', setting = '' } = params;

  const stylePrompts = {
    cartoon: 'cartoon background, simple, colorful, flat design, child-friendly',
    anime: 'anime background, detailed, atmospheric, colorful',
    realistic: '3D rendered background, Pixar style, detailed environment',
    comic: 'comic book background, bold colors, stylized',
  };
  ...
  const prompt = `${stylePrompts[style]}, ${setting} ${description}, ${timePrompts[timeOfDay]}, landscape orientation, no characters, high quality background art`;

  const negativePrompt = 'people, characters, text, watermark, blurry, scary, dark, violent';
